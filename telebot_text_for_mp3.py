...
from parser import get_article_text, get_article_lang, get_link
...
def forward_message(message):
    bot.send_message(message.from_user.id, 'Привет! Я перевожу статьи в аудиофайлы, пришли мне ссылку на статью, '
...
def forward_message(message):
    ...
    if not is_running:
        ...
        if link:  # Проверка, что боту отправили ссылку, а не что-то другое
            ...
            if article_language:  # Проверка, что статья была распарсена и её язык получен
                ...
                bot.send_message(message.from_user.id, "Данная статья не подходит, попробуйте прислать другую...")
            is_running = False
        else:
            bot.send_message(message.from_user.id, 'Нет, это не ссылка, попробуй прислать ссылку на статью.')

    else:
        bot.send_message(message.from_user.id, 'Я занят предыдущим запросом, подожди немного...')
